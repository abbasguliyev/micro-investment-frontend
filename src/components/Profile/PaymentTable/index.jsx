...
const PaymentTable = ({userId}) => {
  ...
  const changePage = (e) => {
    ...
    dispatch(getAllInvestmentsAsync({"investor": userId, "entrepreneur": "", "offset": offset, "amount_must_send__gt": "0.00"}));
  };

  useEffect(() => {
    dispatch(getUserDetailAsync({"id": userId}))

    dispatch(getAllInvestmentsAsync({"investor": userId, "entrepreneur": "", "offset": 0, "amount_must_send__gt": "0.00"}))
    dispatch(getCompanyBalanceAsync())
  }, [dispatch])
